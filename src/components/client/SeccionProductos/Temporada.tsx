...
export default function Temporada({ setColor }: ColorProps) {
    ...
    useEffect(() => {
        if (productos[currentIndex]) {
            setColor(productos[currentIndex].categoria_especials[0]?.color || '#ffaf06');
        }
    ...
    useEffect(() => {
        const fetchProductos = async () => {
            try {
                const res = await fetch("/api/productos-temporada");

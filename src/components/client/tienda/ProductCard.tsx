...
export default function ProductCard({ producto }: { producto: Producto }) {
  ...
  const handleAdd = () => {
    ...
    <ToastPremium
      name={producto.name}
      image={process.env.NEXT_PUBLIC_STRAPI_URL + producto.images[0].url}
    />,
    {
      duration: 2400,
  ...
  return (
    <div className={`${manrope.className} group relative border border-white/60 ${producto.oferta ? 'bg-amber-500/60' : 'bg-gray-200/80'} backdrop-blur-xl rounded-3xl p-4 shadow-lg hover:-translate-y-1 transition-all duration-300`}>
      {producto.oferta ? (
      ...
      <div className="pt-4 text-gray-900">
        ...
        <div className="flex items-center justify-center gap-3 mt-3">
          <button
            onClick={decrementar}
            className="relative w-9 h-9 rounded-full flex items-center justify-center font-semibold text-white text-lg

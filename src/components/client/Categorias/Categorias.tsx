...
export default function Categorias({ categorias }: { categorias: Categoria[] }) {
  ...
  return (
    ...
    <div className="relative mt-6 px-6 rounded-2xl ml-3 mr-3 overflow-hidden shadow-lg">
      ...
      <div className="absolute inset-0 bg-white/50 backdrop-blur-lg border border-white/30 rounded-2xl"></div>

      {/* Contenido */}
      <div className="relative z-10 py-18">
        {/* Título */}
        <h2 className={`${yellowtail.className} text-4xl md:text-6xl font-extrabold text-cyan-950 text-center mb-8`} style={{ textShadow: '2px 2px 8px rgba(0,0,0,0.4)' }}>
          Soluciones para todo
        </h2>
        <p className=' sm:px-10 md:px-20 xl:px-64 text-center pb-20' >Descubre todas nuestras categorías de productos pensadas para tu negocio.
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-8 xl:gap-12 px-4 md:px-12 xl:px-15 2xl:px-30">
          {categorias.map((categoria, index) => {
            ...
            const toggleActiva = (i: number) => {
              // si está activa, la cierra; si no, la abre
            ...
            return (
              <div
                ...
                onClick={() => toggleActiva(index)} // 👉 tocar para expandir en móviles
              >
                {/* Imagen */}
                ...
                {/* Botones (hover en escritorio, clic en móvil) */}
                <div
                  className={`absolute bottom-0 left-0 right-0 p-4 flex flex-col gap-2 
          transition-transform duration-300 ease-out 
          ${activa === index ? "translate-y-0" : "translate-y-full"} 
